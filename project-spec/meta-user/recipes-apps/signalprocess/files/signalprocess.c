...
int main()
{
   ...
   //初始化socket 与上位机进行控制数据通信 
	...
	detectangle.correctphase[0][0] = 300-180;
    detectangle.correctphase[0][1] = 110-180;
    detectangle.correctphase[0][2] = 256-180;
 ...
 printf("start to work\r\n");  
	queue = createQueue();
    networkqueue = networkcreateQueue();
	//线程 信号量初始化
    //天线数据采样
    pthread_t threadsample;
    pthread_t threadgetdata;
    //天线数据处理
    pthread_t threaddatadeal1;
   pthread_t threadnetwork;
    ...
    ret1 = pthread_create(&threadnetwork,NULL, &networkthread,NULL);//创建网络线程
    if(ret1 < 0)
    {
        printf("threadnetwork create error!\n");
        exit(1);
    }
 ...
 ret2 = pthread_create(&threadgetdata,NULL, &getdatathreadrun,NULL);//创建数据get线程1
    if(ret2 < 0)
    {
        printf("threadgetdata create error!\n");
        exit(1);
    }
 sleep(1);
    ret2 = pthread_create(&threaddatadeal1,NULL, &signaldealthreadrun,&args1);//创建数据处理线程1
    if(ret2 < 0)
    {
        printf("threaddatadeal1 create error!\n");
        exit(1);
    }	
	...
	if (((len = recvfrom(server_sockfd, buf, MAXDATASIZE, MSG_DONTWAIT, (struct sockaddr *)&remote_addr, &sin_size)) > 0)) {
		...
		//printf("buf[0]:%d,\r\n",(int)buf[0]);  
			...
			*GPIO_ATEENCTRL = 0x00;
            ...
            if((buf[10]&0xFF)!=0x00)//Unattended mode无人值守模式
            {
                detectangle.mode = 3;
            ...
            if((buf[11]&0xFF)!=0x00)
            {
                ReferenceThresholdPower = (int)(buf[11]&0xFF);
                printf("ReferenceThresholdPower:%d \r\n",ReferenceThresholdPower);
            }
            if((buf[12]&0xFF)==0xD8)
            {
               *GPIO_APPITUDE = 0x00;
            }else if((buf[12]&0xFF)==0xEC)
            {
               *GPIO_APPITUDE = 0x01;
            }else if((buf[12]&0xFF)==0x00)
            {
               *GPIO_APPITUDE = 0x03;
            }
	...
	detectangle.bandwidth = 0.0;//默认带宽1MHz
          ...
          } else  if((buf[5]&0xFF) == 0x01)//宽带信号   //单位20Khz
             {
		 detectangle.mode =2;
		detectangle.frequence =  ((((unsigned short)buf[6]&0xFF)<<8)|((unsigned short)buf[7]&0xFF))*100000.00;
		detectangle.bandwidth = (buf[8]&0xFF)*20000;//单位20Khz
 		 usleep(1000);
    ...
    pthread_join(threaddatadeal1,NULL);//等待线程2退出
