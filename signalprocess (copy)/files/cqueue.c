...
int queue_push(struct QueueInfo *s, struct threadarg *parm)
{
	...
	while (s->next != NULL)
	{
    	...
    	memcpy(temp->parm.ad9226fftdata,parm->ad9226fftdata,fftbuffer_size/2*sizeof(unsigned int )*2*4);
	temp->next = s->next;
	s->next = temp;
	return SUCCESS;
}
...
int queue_pop(struct QueueInfo *s, struct threadarg *parm)
{
	...
	if (queue_is_empty(s))
		return FAILURE;
	/*找出队列顶元素*/
	memcpy(parm->ad9226fftdata,s->next->parm.ad9226fftdata,fftbuffer_size/2*sizeof(unsigned int )*2*4); 
/**拷贝完成释放指针***/
	/*保存等下需要free的指针*/
